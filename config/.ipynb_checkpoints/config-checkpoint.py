...
class Config:
    ...
    SEQ_LEN = 60  # 回看天数
    FEATURE_DIM = 47  # 原始特征维度 (不含行业代码) - 移除ret_1d后变为47维
    D_MODEL = 64  # 模型隐藏维度（从96→64）
    N_LAYERS = 2  # Mamba 层数（从3→2）
    N_HEADS = 4  # 多头注意力头数 (保留兼容性)
    D_STATE = 16  # SSM 状态维度
    D_CONV = 4  # SSM 卷积核大小
    EXPAND = 2  # SSM 扩展因子
    MAMBA_LEVELS = (1,5)  # 多尺度: 仅保留日级，移除周级（从(1,5)→(1,)）
    N_INDUSTRIES = 30  # 实际约111个行业，预留空间 (申万一级行业)
    ...
    BATCH_SIZE = 512  # 进一步减小batch size（从384→64，降低GPU内存占用）
    GRAD_ACCUM_STEPS = 1  # 梯度累积步长，可根据GPU内存调整
    LR_INIT = 1e-3  # 学习率
    WEIGHT_DECAY = 5e-3  # 增加权重衰减（从1e-3→5e-3）
    MAX_EPOCHS = 50  # 最大epoch数
    PATIENCE = 8  # 早停耐心值
    DROPOUT = 0.2  # 增加dropout比例（从0.1→0.2）
    SLIDE_STEP = 1  # 滑动窗口步长，可根据需要调整，默认为5，平衡速度和性能
