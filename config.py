...
class Config:
    ...
    SEQ_LEN = 60  # 回看天数
    FEATURE_DIM = 76  # 扩展特征维度 (原始特征 + 新增技术指标 + 波动率特征 + 动量特征 + 市场状态特征)
    D_MODEL = 128  # 模型隐藏维度（从96→128）
    N_LAYERS = 4  # Mamba 层数（从3→4）
    N_TRANSFORMER_LAYERS = 3  # Transformer 层数（从2→3）
    N_HEADS = 8  # 多头注意力头数 (从6→8，确保128能被8整除)
    D_STATE = 64  # SSM 状态维度（从32→64）
    D_CONV = 4  # SSM 卷积核大小
    EXPAND = 2  # SSM 扩展因子
    MAMBA_LEVELS = (1, 5)  # 多尺度: 日级和周级
    N_INDUSTRIES = 111  # 实际约111个行业，预留空间 (申万一级行业)
    USE_GRAPH = True  # 使用行业嵌入
    N_GCN_LAYERS = 2  # GCN层数
    PATCH_LEN = 12  # 保留兼容性
    STRIDE = 8  # 保留兼容性

    # ============ Training (Optimized for Strong Generalization) ============
    TRAIN_YEARS = 2  # 使用2年数据训练（与 TRAIN_START_YEARS 保持一致，从3→2）
    BATCH_SIZE = 32  # 进一步减小batch size至32，适应6GB GPU显存
    GRAD_ACCUM_STEPS = 4  # 梯度累积步长，模拟更大的批次大小 (384 * 4 = 1536)
    LR_INIT = 5e-4  # 学习率（从1e-3→5e-4，更深的模型需要更小的学习率）
    WEIGHT_DECAY = 3e-3  # 权重衰减（从5e-3→3e-3，平衡正则化和模型表达能力）
    MAX_EPOCHS = 100  # 最大epoch数（从50→100，增加训练轮数）
    PATIENCE = 15  # 早停耐心值（从8→15，给予模型更多训练时间）
    DROPOUT = 0.25  # 增加dropout比例（从0.2→0.25，防止过拟合）
    SLIDE_STEP = 3  # 滑动窗口步长，可根据需要调整，默认为5，平衡速度和性能
