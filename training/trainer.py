...
if __name__ == "__main__":
    ...
    try:
        ...
        # 1. 初始化数据模块
        logger.info("1. 初始化数据模块...")
        logger.info(f"   数据根目录: {Config.DATA_ROOT}")
        logger.info(f"   特征维度: {Config.FEATURE_DIM}")
        logger.info(f"   序列长度: {Config.SEQ_LEN}")
        
        # 2. 检查股票数据
        logger.info("2. 检查股票数据...")
        from data.downloader import YahooDownloader
        downloader = YahooDownloader()
        
        # 获取主板股票列表
        stock_list = downloader.get_main_board_stocks()
        logger.info(f"   主板股票数量: {len(stock_list)}")
        ...
        if raw_data_dir.exists():
            for file in raw_data_dir.iterdir():
                if file.suffix == '.parquet':
                    ts_code = file.stem.replace('_', '.')
                    cached_stocks.add(ts_code)
        
        logger.info(f"   本地缓存股票数量: {len(cached_stocks)}")
        
        # 计算需要下载的股票
        stock_codes = stock_list['ts_code'].tolist()
        stocks_to_download = [code for code in stock_codes if code not in cached_stocks]
        
        if stocks_to_download:
            logger.info(f"   需要下载的股票数量: {len(stocks_to_download)}")
            logger.info(f"   开始下载缺失的股票数据...")
            
            # 分批下载，避免并发过高
            batch_size = 10
            for i in range(0, len(stocks_to_download), batch_size):
                batch = stocks_to_download[i:i+batch_size]
                logger.info(f"   下载批次 {i//batch_size + 1}/{(len(stocks_to_download) + batch_size - 1)//batch_size}")
                downloader.download_all(stock_list=batch, force_update=False)
        else:
            logger.info("   所有股票数据已缓存，无需下载")
        
        # 3. 创建数据模块
        logger.info("3. 创建数据模块...")
        data_module = AlphaDataModule(
            force_rebuild=True,  # 强制重建缓存，确保使用最新数据
            auto_clear_cache=True,  # 自动清除旧缓存
            memory_optimized=True  # 启用内存优化模式
        )
        
        # 4. 检查数据加载器
        logger.info("4. 检查数据加载器...")
        train_loader = data_module.train_dataloader()
        val_loader = data_module.val_dataloader()
        
        logger.info(f"   训练集批次大小: {train_loader.batch_size}")
        logger.info(f"   验证集批次大小: {val_loader.batch_size}")
        logger.info(f"   训练集加载线程: {train_loader.num_workers}")
        logger.info(f"   验证集加载线程: {val_loader.num_workers}")
        
        # 5. 初始化训练器
        logger.info("5. 初始化训练器...")
        logger.info(f"   模型维度: {Config.D_MODEL}")
        logger.info(f"   层数: {Config.N_LAYERS}")
        logger.info(f"   注意力头数: {Config.N_HEADS}")
        logger.info(f"   状态维度: {Config.D_STATE}")
        logger.info(f"   Mamba 级别: {Config.MAMBA_LEVELS}")
        ...
        # 6. 启动训练
        logger.info("6. 启动训练...")
        logger.info(f"   最大训练轮数: {trainer.max_epochs}")
        logger.info(f"   学习率: {trainer.lr}")
        logger.info(f"   设备: {trainer.device}")
        logger.info(f"   混合精度: {trainer.use_amp}")
        logger.info(f"   梯度累积步数: {trainer.grad_accum_steps}")
        logger.info(f"   早停耐心值: {trainer.patience}")
